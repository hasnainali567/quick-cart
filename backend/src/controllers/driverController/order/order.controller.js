...
export const acceptDelivery = asynHandler(async (req, res) => {
  ...
  notify({
    ...
    data: {
      ...
      body: `Driver ${driver.user.name} is heading to the store`,
    },
  });

  return res.json(
    new ApiResponse(200, delivery, "Delivery accepted successfully"),
  );
});
...
export const rejectDelivery = asynHandler(async (req, res) => {
  ...
  return res.json(
    new ApiResponse(200, null, "Delivery rejected — finding another driver"),
  );
});
...
export const pickedUpOrder = asynHandler(async (req, res) => {
  ...
  return res.json(
    new ApiResponse(200, result, "Order picked up — out for delivery"),
  );
});
...
export const requestToPickup = asynHandler(async (req, res) => {
  ...
  const notification = await notify({
    ...
    data: {
      title: "🔔 New Delivery",
      body: `New delivery resquest from ${order.store.name}`,
      type: "ORDER_READY",
    },
  });
  ...
  return new ApiResponse(200, null, "Driver notified for pickup").send(
    res,
  );
});
