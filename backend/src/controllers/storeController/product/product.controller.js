...
export const getAllProducts = asynHandler(async (req, res) => {
    ...
    return new ApiResponse(200, { products, totalPages, productCount, hasNextPage }, 'Products fetched successfully').send(res)
})
...
export const createProduct = asynHandler(async (req, res) => {
    ...
    const imageUrls = uploadedImages.map((image) => image.secureUrl || image.secure_url)
                    ...
                    variants: parsedVariants.length
                        ? {
                            create: parsedVariants.map((variant) => ({
                                ...
                                salePrice: variant.salePrice !== undefined && variant.salePrice !== null ? Number(variant.salePrice) : undefined,
...
export const updateProduct = asynHandler(async (req, res) => {
    ...
    const { name, description, categoryId, unit, price, salePrice, costPrice, tags, weight } = req.body;
    ...
    const updatedProduct = await prisma.product.update({
        ...
        data: {
            ...
            salePrice: salePrice !== undefined ? Number(salePrice) : undefined,
            costPrice: costPrice !== undefined ? Number(costPrice) : undefined,
            tags: tags ? (Array.isArray(tags) ? tags : tags.split(',').map((tag) => tag.trim()).filter(Boolean)) : undefined,
            weight: weight !== undefined ? Number(weight) : undefined,
        }
    })

    return new ApiResponse(200, updatedProduct, 'Product updated successfully').send(res)
})
...
export const toggleProductStatus = asynHandler(async (req, res) => {
    ...
    return new ApiResponse(200, updatedProduct, `Product ${updatedProduct.isActive ? 'activated' : 'deactivated'} successfully`).send(res)
})
...
export const updateProductVariant = asynHandler(async (req, res) => {
    ...
    return new ApiResponse(200, updatedVariant, 'Variant updated successfully').send(res)
})
...
export const toggleProductVariantStatus = asynHandler(async (req, res) => {
    ...
    return new ApiResponse(200, updatedVariant, `Variant ${updatedVariant.isActive ? 'activated' : 'deactivated'} successfully`).send(res)
})
...
export const getProductVariant = asynHandler(async (req, res) => {
    ...
    return new ApiResponse(200, variant, 'Variant fetched successfully').send(res)
})
